...
const countVisibleEntities = `-- name: CountVisibleEntities :one
SELECT
...
type CountVisibleEntitiesParams struct {
...
type CountVisibleEntitiesRow struct {
	Countries int32 `json:"countries"`
	Regions   int32 `json:"regions"`
	Districts int32 `json:"districts"`
}
...
func (q *Queries) CountVisibleEntities(ctx context.Context, arg CountVisibleEntitiesParams) (CountVisibleEntitiesRow, error) {
	row := q.db.QueryRow(ctx, countVisibleEntities,
		arg.StMakeenvelope,
	...
	var i CountVisibleEntitiesRow
	err := row.Scan(&i.Countries, &i.Regions, &i.Districts)
	return i, err
}
...
func (q *Queries) GetBoundaryImportsByLevel(ctx context.Context, levelID int16) ([]GetBoundaryImportsByLevelRow, error) {
	rows, err := q.db.Query(ctx, getBoundaryImportsByLevel, levelID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const getCityBoundariesByIDs = `-- name: GetCityBoundariesByIDs :many
SELECT
    c.id,
    c.name,
...
type GetCityBoundariesByIDsRow struct {
	ID              int32       `json:"id"`
	Name            string      `json:"name"`
	CountryCode     *string     `json:"country_code"`
...
func (q *Queries) GetCityBoundariesByIDs(ctx context.Context, dollar_1 []int32) ([]GetCityBoundariesByIDsRow, error) {
	rows, err := q.db.Query(ctx, getCityBoundariesByIDs, dollar_1)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetCityBoundariesByIDsRow{}
	for rows.Next() {
		var i GetCityBoundariesByIDsRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.CountryCode,
...
func (q *Queries) GetCityBoundaryByID(ctx context.Context, id int32) (GetCityBoundaryByIDRow, error) {
...
func (q *Queries) GetNameMappingsByLevel(ctx context.Context, levelID int16) ([]GetNameMappingsByLevelRow, error) {
	rows, err := q.db.Query(ctx, getNameMappingsByLevel, levelID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
type LookupAllLevelsByPointWithAreaRow struct {
	...
	DistrictCount     int32    `json:"district_count"`
	DistrictsInRegion int32    `json:"districts_in_region"`
}
