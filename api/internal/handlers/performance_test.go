...
func TestLocationOverrideLookupPerformance(t *testing.T) {
	...
	// Test location override lookup (GetLocationOverrideForCalculation)
	// Note: This will likely return no rows, but we're testing the query performance
	start := time.Now()
	override, err := database.Queries.GetLocationOverrideForCalculation(ctx, sqlcgen.GetLocationOverrideForCalculationParams{
		PublisherID: publisherID,
		CityID:      293397, // Jerusalem
	})
	duration := time.Since(start)

	// sql.ErrNoRows is expected if no override exists - this is OK for performance testing
	hasOverride := err == nil
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		t.Fatalf("GetLocationOverrideForCalculation failed: %v", err)
	}
	if hasOverride {
		t.Logf("Found override: lat=%v, lon=%v", override.OverrideLatitude, override.OverrideLongitude)
	}
	...
	testCities := []int32{293397, 5128581, 2643743, 3448439} // Jerusalem, NYC, London, Sao Paulo

	for _, cityID := range testCities {
		start := time.Now()
		_, lookupErr := database.Queries.GetLocationOverrideForCalculation(ctx, sqlcgen.GetLocationOverrideForCalculationParams{
			PublisherID: publisherID,
			CityID:      cityID,
		})
		duration := time.Since(start)
		...
		if lookupErr != nil && !errors.Is(lookupErr, sql.ErrNoRows) {
			t.Errorf("GetLocationOverrideForCalculation failed for city %d: %v", cityID, lookupErr)
		}

		if duration > 20*time.Millisecond {
			t.Errorf("GetLocationOverrideForCalculation too slow for city %d: %v (target: < 20ms)", cityID, duration)
		}

		t.Logf("  City %d: %v", cityID, duration)
	}
}
...
func BenchmarkLocationOverrideLookup(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err := database.Queries.GetLocationOverrideForCalculation(ctx, sqlcgen.GetLocationOverrideForCalculationParams{
			PublisherID: publisherID,
			CityID:      293397,
		})
		if err != nil && !errors.Is(err, sql.ErrNoRows) {
