...
export const PRIMITIVES_DOCUMENTATION: Record<string, PrimitiveDocumentation> = {
  ...
  sunrise: {
    ...
    astronomicalDefinition: 'The moment when the sun\'s center reaches 0° altitude (geometric horizon). The standard zenith angle used is 90.833°, which includes a correction of 0.833° for atmospheric refraction at the horizon and the sun\'s semi-diameter.',
    ...
    algorithm: {
      ...
      steps: [
        ...
        { step: 5, description: 'Compute hour angle (HA) for zenith = 90.833°', formula: 'HA = arccos((cos(90.833°) / (cos(lat) × cos(δ))) - tan(lat) × tan(δ))' },
    ...
    halachicSignificance: 'This astronomical primitive is commonly used as the basis for calculating "Netz HaChama" and marks the beginning of the halachic day according to the GRA system. It is the starting point for calculating sha\'os zmaniyos (proportional hours) in many calculation methods.',
    ...
    practicalNotes: 'The calculated geometric sunrise differs from visible sunrise by 2-4 minutes depending on latitude and atmospheric conditions. Mountains or buildings on the eastern horizon can delay the actual visible moment.',
  ...
  sunset: {
    ...
    scientificExplanation: 'Geometric sunset occurs when the geometric center of the sun crosses the geometric horizon (0° altitude) in the evening. Due to atmospheric refraction, the sun remains visible for about 2-3 minutes after this calculated moment.',

    astronomicalDefinition: 'The moment when the sun\'s center reaches 0° altitude on the western horizon. Uses the same 90.833° zenith angle as sunrise, accounting for atmospheric refraction and solar semi-diameter.',

    algorithm: {
      ...NOAA_ALGORITHM_BASE,
      description: 'Same NOAA algorithm as sunrise, but calculating when the sun crosses the horizon in the evening.',
      steps: [
        { step: 1, description: 'Convert date to Julian Day (JD)' },
        { step: 2, description: 'Calculate Julian Century (T) from J2000.0 epoch' },
        { step: 3, description: 'Compute solar declination (δ) using orbital elements' },
        { step: 4, description: 'Calculate equation of time (EoT) for true solar time' },
        { step: 5, description: 'Compute hour angle (HA) for zenith = 90.833°' },
    ...
    halachicSignificance: 'This astronomical primitive is commonly used as the basis for calculating "Shkiah" and marks the end of the halachic day. It defines the start of the twilight period (bein hashmashos) and is the reference point for Shabbos/holiday start times.',
    ...
    practicalNotes: 'The exact moment of visible sunset can vary based on terrain features like mountains on the western horizon. The calculated geometric sunset differs from visible sunset by 2-3 minutes.',

    accuracy: '±1 minute under normal atmospheric conditions.',

    edgeCases: 'Polar regions may have days without sunset. Mountainous terrain can significantly affect the apparent sunset time.',
  ...
  sunrise_visible: {
    ...
    astronomicalDefinition: 'The moment when the sun\'s upper limb reaches the apparent horizon, calculated with a zenith angle of approximately 90° (the sun\'s center is at -0.833° geometric altitude: 0.567° refraction + 0.266° semi-diameter).',

    algorithm: {
      ...NOAA_ALGORITHM_BASE,
      description: 'Same NOAA algorithm with adjusted zenith to account for the sun\'s upper limb and full refraction correction.',
      steps: [
        { step: 1, description: 'Calculate using standard algorithm but with zenith = 90.0° (for upper limb visibility)' },
        { step: 2, description: 'The 0.833° difference from geometric sunrise accounts for refraction + semi-diameter' },
        { step: 3, description: 'Result is approximately 2-4 minutes before geometric sunrise' },
      ],
    },

    halachicSignificance: 'This primitive represents when sunlight first reaches the observer, accounting for atmospheric refraction. Some poskim use this rather than geometric sunrise for practical zmanim calculations.',

    halachicSources: [
      { source: 'Bi\'ur Halacha', reference: '58:1', description: 'Discusses when the sun is considered to have risen for halachic purposes' },
      { source: 'Mishnah Berurah', reference: '588:1 (regarding Vasikin)', description: 'The preferred practice of praying at first light' },
    ],

    practicalNotes: 'Occurs 2-4 minutes before geometric sunrise. Temperature and humidity affect the exact amount of refraction. Clear, cold mornings may have different refraction than humid summer mornings.',

    accuracy: '±2 minutes due to variable atmospheric refraction conditions.',
  ...
  sunset_visible: {
    ...
    astronomicalDefinition: 'When the sun\'s upper limb reaches 0° apparent altitude (approximately -0.833° true altitude after refraction correction).',

    algorithm: {
      ...NOAA_ALGORITHM_BASE,
      description: 'Same NOAA algorithm with adjusted zenith for visible upper limb.',
      steps: [
        { step: 1, description: 'Calculate using standard algorithm with zenith = 90.0°' },
        { step: 2, description: 'Result is approximately 2-4 minutes after geometric sunset' },
      ],
    },

    halachicSignificance: 'This primitive represents when the sun is no longer visible to an observer, accounting for atmospheric refraction. Some poskim use this rather than geometric sunset for practical calculations.',

    halachicSources: [
      { source: 'Shulchan Aruch', reference: 'Orach Chaim 261', description: 'Discussion of when Shabbos begins relative to sunset' },
    ],

    practicalNotes: 'Occurs 2-4 minutes after geometric sunset. Mountains or tall buildings on the western horizon can delay this further. The exact time varies with atmospheric conditions.',

    accuracy: '±2 minutes due to variable atmospheric conditions.',

    authoritativeLinks: [
      { title: 'US Naval Observatory', url: 'https://aa.usno.navy.mil/', description: 'Official astronomical data source' },
    ],
  },
