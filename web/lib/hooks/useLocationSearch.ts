...
export interface UseLocationSearchOptions {
  /** API endpoint to search (e.g., '/cities', '/coverage/search') */
  endpoint: string;
 ...
 *       {results.map(city => <div key={city.id}>{city.name}</div>)}
 *     </div>
 *   );
 * }
 * ```
 *
 * @example Coverage search with filters
 * ```tsx
 * function CoverageSearch() {
 *   const { query, setQuery, results, loading } = useLocationSearch({
 *     endpoint: '/coverage/search',
 *     debounceMs: 300,
 *     filters: { levels: 'city,region', limit: 20 },
 *   });
 *
 *   return (
 *     <div>
  ...
  useEffect(() => {
    ...
    const performSearch = async () => {
      ...
      try {
        ...
        // Some endpoints return { cities: [...] }, others { results: [...] }
        let searchResults: T[] = [];
        if (Array.isArray(data)) {
          searchResults = data;
        } else if (data.cities) {
          searchResults = data.cities;
